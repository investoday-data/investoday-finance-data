...
接口说明：通过股票代码或代码列表及可选的发布日期范围，批量获取沪深京股票的财务衍生指标数据，涵盖盈利能力（如毛利率、ROE）、成长能力（如营收增长率）、偿债能力（如流动比率、Z值）、营运能力（如周转率）及现金流等核心维度，返回包含股票基本信息、报告期截止日及各指标数值的详细列表，适用于大模型进行深度的企业基本面分析、财务健康度评估及投资价值挖掘。
